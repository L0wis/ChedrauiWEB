...
try {
    ...
    $sql = "SELECT * FROM compra_personal WHERE id_transaccion = :id_transaccion";
    ...
    if ($venta) {
        ...
        $pdf->Cell(0, 10, utf8_decode('ID Transacción: ') . $venta['id_transaccion'], 0, 1);
        $pdf->Cell(0, 10, 'Fecha: ' . $venta['fecha'], 0, 1);
        $pdf->Cell(0, 10, 'Status: ' . $venta['status'], 0, 1);
        $pdf->Cell(0, 10, 'Email: ' . $venta['email'], 0, 1);
        $pdf->Cell(0, 10, 'ID Cliente: ' . $venta['id_cliente'], 0, 1);
        ...
        if ($cliente) {
            $pdf->Cell(0, 10, 'Nombre Cliente: ' . $cliente['nombres'] . ' ' . $cliente['apellidos'], 0, 1);
        }
        $pdf->Cell(0, 10, 'Total: ' . $venta['total'], 0, 1);
