...
function emailExiste($email, $con){
    ...
    $msg = "No existe el registro del cliente.";
        ...
        function login($usuario, $password, $con){
            $sql = $con->prepare("SELECT id, usuario, password, nombre FROM personal WHERE usuario LIKE ? AND activo = 1 LIMIT 1");
            $sql->execute([$usuario]);
            if($row = $sql->fetch(PDO::FETCH_ASSOC)){
                   if(password_verify($password, $row['password'])){
                    $_SESSION['user_id'] = $row['id'];
                    $_SESSION['user_name'] = $row['nombre'];
                    $_SESSION['user_type'] = 'pearson';  /* USUARIO PARA PERSONAL */
                    $_SESSION['user_type'] = 'ElAmanteDelCafe';  /* USUARIO PARA BEBIDAS */
                    $_SESSION['user_type'] = 'ChefGourmet';  /* USUARIO PARA COMIDA */
                    $_SESSION['user_type'] = 'SrLimpios';  /* USUARIO PARA LIMPIEZA */
                    $_SESSION['user_type'] = 'FashionMan';  /* USUARIO PARA ROPA PARA CABALLERO */
                    $_SESSION['user_type'] = 'StyleQueen';  /* USUARIO PARA ROPA PARA DAMA */
                    $_SESSION['user_type'] = 'TechGeek';  /* USUARIO PARA ELECTRONICA */
                    $_SESSION['user_type'] = 'CleanFreak';  /* USUARIO PARA HIGIENE */
